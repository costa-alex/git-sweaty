...
- Keep your personal workflow-generated outputs on `dashboard-data` (automatic).
- Keep `main` focused on template code/config/docs.
- Use GitHub's **Sync fork** on `main` to pull upstream changes with minimal merge conflicts.
- After syncing upstream, run [Sync Strava Heatmaps](../../actions/workflows/sync.yml) once to refresh generated outputs if needed.
